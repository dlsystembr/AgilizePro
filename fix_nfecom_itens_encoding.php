 ...
 * 
 * Uso:
 * php fix_nfecom_itens_encoding.php
 * ou acesse via navegador: http://localhost/mapos/fix_nfecom_itens_encoding.php
 */
...
try {
    ...
    if ($is_web) {
        echo "<!DOCTYPE html><html><head><meta charset='UTF-8'><title>Correção de NFCom Itens</title>";
        echo "<style>
        ...
        echo "<h1>Correção de Encoding na Tabela NFCom Itens</h1>";
    }
    ...
    foreach ($correcoes as $correcao) {
        ...
        $stmt = $mysqli->prepare("SELECT nfi_id, nfc_id, nfi_x_prod FROM nfecom_itens WHERE {$campo} LIKE ? LIMIT 20");
    ...
    foreach ($correcoes as $correcao) {
        list($texto_errado, $texto_correto) = $correcao;
        $stmt = $mysqli->prepare("SELECT COUNT(*) as total FROM nfecom_itens WHERE {$campo} LIKE ?");
        $pattern = "%{$texto_errado}%";
        $stmt->bind_param("s", $pattern);
        $stmt->execute();
        $result = $stmt->get_result();
    ...
    if ($is_web) {
        if (!empty($problemas)) {
            ...
            echo "<table>";
            echo "<tr><th>ID</th><th>NFCom ID</th><th>Texto Errado</th><th>Texto Correto</th><th>Descrição Produto</th></tr>";
            foreach (array_slice($problemas, 0, 50) as $problema) {
                ...
                echo "<td><pre>" . htmlspecialchars($problema['nfi_x_prod']) . "</pre></td>";
                echo "</tr>";
            }
            echo "</table>";
        ...
        if (!empty($problemas)) {
            ...
            foreach (array_slice($problemas, 0, 10) as $problema) {
                echo "ID {$problema['id']} (NFCom: {$problema['nfc_id']})\n";
                echo "  Errado: '{$problema['texto_errado']}' -> Correto: '{$problema['texto_correto']}'\n";
                echo "  Produto: {$problema['nfi_x_prod']}\n\n";
            }
        } else {
    ...
    if ($total_afetados > 0) {
        ...
        foreach ($correcoes as $correcao) {
            ...
            $stmt = $mysqli->prepare("UPDATE nfecom_itens SET {$campo} = REPLACE({$campo}, ?, ?), nfi_data_atualizacao = NOW() WHERE {$campo} LIKE ?");
            $pattern = "%{$texto_errado}%";
