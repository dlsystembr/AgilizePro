...
class Mapos_model extends CI_Model
{
    ...
    public function getOsStatus($status)
    {
        ...
        if (!$query) {
            log_message('error', 'Erro na query getOsStatus: ' . $this->db->error()['message']);
            return array();
        }
    ...
    public function getEmitente()
    {
        ...
        if ($emitente) {
            ...
            $requiredFields = ['ie', 'ibge', 'nome', 'cnpj', 'rua', 'numero', 'bairro', 'cidade', 'uf', 'cep', 'telefone', 'email'];
            foreach ($requiredFields as $field) {
                if (!isset($emitente->$field)) {
                    $emitente->$field = '';
                }
            }
        }
    ...
    public function addEmitente($nome, $cnpj, $ie, $cep, $logradouro, $numero, $bairro, $cidade, $uf, $ibge, $telefone, $email, $url_logo)
    {
    ...
    public function editEmitente($id, $nome, $cnpj, $ie, $cep, $logradouro, $numero, $bairro, $cidade, $uf, $ibge, $telefone, $email)
    {
        $this->db->set('nome', $nome);
        $this->db->set('cnpj', $cnpj);
        $this->db->set('ie', !empty($ie) ? $ie : 'ISENTO');
        $this->db->set('cep', $cep);
        $this->db->set('rua', $logradouro);
        $this->db->set('numero', $numero);
        $this->db->set('bairro', $bairro);
        ...
        try {
            ...
            foreach ($data as $key => $valor) {
                ...
                if ($exists) {
                    ...
                    // Compatibilidade com colunas antigas (VARCHAR(20)) que podem ter truncado a chave
                    $legacyKey = mb_substr($key, 0, 20, 'UTF-8');
                    $this->db->where('config', $legacyKey);
                    $legacy = $this->db->get('configuracoes')->row();
            ...
            if ($this->db->trans_status() === FALSE) {
                log_message('error', 'Erro ao salvar configurações: ' . $this->db->error()['message']);
                return false;
            }
            
            return true;
        } catch (Exception $e) {
            log_message('error', 'Exception ao salvar configurações: ' . $e->getMessage());
            return false;
        }
    }
    ...
    public function initConfiguracoes()
    {
        ...
        foreach ($defaultConfigs as $key => $valor) {
            $this->db->where('config', $key);
            $exists = $this->db->get('configuracoes')->row();
