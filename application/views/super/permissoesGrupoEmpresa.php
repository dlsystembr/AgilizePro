...
<div class="new122">
    <div class="widget-title" style="margin: -15px -10px 0">
        ...
        <h5>Permissões do grupo: <?= htmlspecialchars($grupo->gpu_nome) ?></h5>
    </div>
    <p style="margin: 8px 0 16px; color: #666;">Empresa: <?= htmlspecialchars($empresa->emp_razao_social ?? $empresa->emp_nome_fantasia ?? 'Empresa') ?> — Grupo: <?= htmlspecialchars($grupo_empresarial->gre_nome) ?></p>
    <a href="<?= base_url("index.php/super/usuariosEmpresa/{$grupo_empresarial->gre_id}/{$empresa->emp_id}") ?>" class="button btn btn-mini btn-warning" style="max-width: 120px">
    ...
    <?php if (!$tem_menus): ?>
        <div class="alert alert-warning" style="margin-top: 15px;">
            Nenhum menu liberado para esta empresa. Configure os menus em <strong>Super &gt; Empresas &gt; Menus da empresa</strong>.
        </div>
    <?php else: ?>
        <form action="<?= base_url('index.php/super/salvarPermissoesGrupoEmpresa') ?>" method="post" id="formPermissoes">
            <input type="hidden" name="gpu_id" value="<?= (int) $grupo->gpu_id ?>" />
            <input type="hidden" name="grupo_id" value="<?= (int) $grupo_empresarial->gre_id ?>" />
            <input type="hidden" name="empresa_id" value="<?= (int) $empresa->emp_id ?>" />
                    ...
                    <table class="table table-bordered perm-table perm-table-cadastro">
                        ...
                        <tbody>
                            <?php foreach ($menus_cadastro as $me): ?>
                                ...
                                ?>
                                <tr>
                                    <td><strong><?= htmlspecialchars($me->men_nome) ?></strong></td>
                                    <td><label class="perm-label"><input type="checkbox" name="perm[<?= $mep_id ?>][visualizar]" value="1" class="perm-checkbox perm-cadastro perm-visualizar" <?= $pa['gup_visualizar'] ? ' checked="checked"' : '' ?> /></label></td>
                                    <td><label class="perm-label"><input type="checkbox" name="perm[<?= $mep_id ?>][editar]" value="1" class="perm-checkbox perm-cadastro perm-editar" <?= $pa['gup_editar'] ? ' checked="checked"' : '' ?> /></label></td>
                                    <td><label class="perm-label"><input type="checkbox" name="perm[<?= $mep_id ?>][deletar]" value="1" class="perm-checkbox perm-cadastro perm-deletar" <?= $pa['gup_deletar'] ? ' checked="checked"' : '' ?> /></label></td>
                                    <td><label class="perm-label"><input type="checkbox" name="perm[<?= $mep_id ?>][alterar]" value="1" class="perm-checkbox perm-cadastro perm-alterar" <?= $pa['gup_alterar'] ? ' checked="checked"' : '' ?> /></label></td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
                    ...
                    <table class="table table-bordered perm-table perm-table-relatorio">
                        ...
                        <tbody>
                            <?php foreach ($menus_relatorio as $me): ?>
                                ...
                                <tr>
                                    <td><strong><?= htmlspecialchars($me->men_nome) ?></strong></td>
                                    <td><label class="perm-label"><input type="checkbox" name="perm[<?= $mep_id ?>][relatorio]" value="1" class="perm-checkbox perm-relatorio-only perm-relatorio" <?= !empty($pa['gup_relatorio']) ? ' checked="checked"' : '' ?> /></label></td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
            ...
            <div class="form-actions">
                ...
                <a href="<?= base_url("index.php/super/usuariosEmpresa/{$grupo_empresarial->gre_id}/{$empresa->emp_id}") ?>" class="button btn btn-warning">
                    <span class="button__icon"><i class="bx bx-undo"></i></span>
                    <span class="button__text2">Voltar</span>
                </a>
            </div>
